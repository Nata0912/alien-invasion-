...
class Scoreboard:
  ''' Клас що показує рахунок у грі'''

  def __init__(self, ai_game):
    ''' Ініціація атрибутів пов'язаних із рахунком'''
  ...
  def prep_high_score(self):
    ''' Перетворює останній рекорд у зображення '''
  ...
  def check_high_score(self):
    ''' Перевіряє чи не встановлено новий рекорд'''
    if self.stats.score > self.stats.high_score:
      self.stats.high_score = self.stats.score
      self.prep_high_score()

  def prep_level(self):
    ''' Поретворює рівень у зображенння'''
